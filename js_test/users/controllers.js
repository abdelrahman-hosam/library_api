...
const create_new_user = async (req , res) => {
    ...
    if(!username || !password) return res.status(400).json({'failure message': 'username and password are required'})
    const [is_repeated] = await database.promise().query('SELECT username FROM users WHERE username = ?' , [username])
    if(is_repeated.length > 0) return res.status(400).json({'failure message': 'username already exists'})
    const hashedpass = bcrypt.hashSync(password , 10)
    await database.promise().query(`INSERT INTO users (username , password , author_id) VALUES(? , ?)` , [username , hashedpass])
...
const login_user = async (req , res) => {
    ...
    if(user.length === 0) return res.status(404).json({'failure message':'user does not exist'})
    ...
    else return res.status(400).json({'failure message':'password is incorrect'})
}
