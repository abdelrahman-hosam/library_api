...
const get_book = async (req , res) =>{
    ...
    if(books.length === 0) return res.status(200).json({'error message': 'there are no books with the title(s) you inserted try other titles'})
...
const add_book = async (req , res) => {
    const book_title = req.body.book_title , authors = req.body.authors
...
const borrow_book =async (req , res) =>{
    ...
    if(book_status !== 'available') return res.status(200).json({'error message': 'The book is not available'})
